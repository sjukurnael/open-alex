...
- **Render persistent disk for SQLite** — simple and cheap, but means the DB lives on a single instance. If horizontal scaling is needed, migrate to a hosted database. The disk is mounted at `/data` (not `/app`) to avoid conflicts with the container's working directory — a common Docker/Render gotcha where mounting a disk at the same path as `WORKDIR` causes the disk to shadow or be shadowed by the container filesystem on each deploy.
...
- **Render Starter tier** — Currently on the Starter paid tier ($7/month) to avoid free-tier spin-down. Worth reviewing for cost if the service scales.
- **Load-once strategy for full load** — `load.py` checks `is_empty()` on startup and only runs if the database has no rows. This ensures the full 572k trial load only happens once on first deploy, and subsequent restarts/redeploys skip it entirely. The downside is that if the persistent disk is ever wiped, the full load will re-run automatically on next startup (~15 minutes of downtime before data is available). Known limitation: if the disk is partially corrupted and only a few rows survive, `is_empty()` returns false and `load.py` never re-runs, leaving the DB permanently underpopulated. A more robust solution would replace `is_empty()` with a threshold check (e.g. fewer than 500k rows triggers a reload), but this is considered over-engineering for the current use case.
